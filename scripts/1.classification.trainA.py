...
for neg_sampling in [1]:#range(1,5):
    data_config_paths = []
    for lang in train_langs:
        setup_name = 'taska.' + lang + '.neg' + str(neg_sampling)
        train_path = '../data/' + setup_name
        dev_path = '../data/taska.' + lang + '.dev'
        config = {}
    ...
    for lm in myutils.llms + myutils.sllms:
        model_name = 'multilingual.' + lm.split('/')[-1] + '.' + str(neg_sampling)
